...
export MY_CMAKE_FLAGS="${MY_CMAKE_FLAGS} -DBUILD_SHARED_LIBS=OFF -DLINKSTATIC=ON -DOpenJPEG_ROOT=/root/ext/dist/lib/ -DOIIO_BUILD_TESTS=OFF -DLIBHEIF_INCLUDE_PATH=/root/ext/dist/include -DLIBHEIF_LIBRARY_PATH=/root/ext/dist/lib -DOPENCOLORIO_NO_CONFIG=ON -DOpenColorIO_ROOT=/root/ext/dist"
